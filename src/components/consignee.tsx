...
import ConsigneeDrawer from "@/components/drawer/ConsigneeDrawer";
...
export const ConsigneeManagement = () => {
      ...
      <div className="bg-gray-50 min-h-screen">
        <div className="bg-white border-b border-gray-200 px-4 py-3">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-xl font-semibold text-gray-900">Consignee</h1>
              <p className="text-xs text-gray-600 mt-0.5">
                Manage your supply chain Consignees
              </p>
            </div>
          </div>
        </div>
  ...
  const handleSave = (data: { code: string; title: string }) => {
    if (editingConsignee) {
      ...
      setConsignees((prev) =>
        prev.map((Consignee) =>
          Consignee.id === editingConsignee.id
            ? { ...Consignee, ...data }
            : Consignee
        )
      );
    } else {
      // Create new Consignee with incremental ID
  ...
  const handleDelete = (id: number) => {
    if (confirm("Are you sure you want to delete this Consignee?")) {
      setConsignees((prev) => prev.filter((Consignee) => Consignee.id !== id));
    }
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Consignees</h1>
            <p className="text-xs text-gray-600 mt-0.5">
              Manage your supply chain Consignees
            </p>
          </div>
          ...
          <ConsigneeDrawer
            ...
            onSave={handleSave}
            editingConsignee={editingConsignee}
          />
        </div>
      </div>
          ...
          {Consignees.length === 0 ? (
            <div className="text-center py-8 px-4">
              ...
              <p className="text-xs text-gray-500 mb-4 max-w-sm mx-auto">
                Add Consignees to organize your supply chain locations.
              </p>
              <ConsigneeDrawer
                isDrawerOpen={isDrawerOpen}
                setIsDrawerOpen={setIsDrawerOpen}
                  ...
                  <TableBody>
                    {Consignees.map((Consignee, index) => (
                      ...
                      >
                        ...
                        <TableCell className="px-4 py-2.5">
                          <div className="text-xs text-gray-500">
                            {Consignee.createdAt.toLocaleDateString("en-US", {
                              month: "short",
                              day: "numeric",
                              year: "numeric",
                            })}
