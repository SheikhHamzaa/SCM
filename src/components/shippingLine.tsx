...
import ShippingLineDrawer from "@/components/drawer/ShippingLineDrawer";
...
export const ShippingLineManagement = () => {
      ...
      <div className="bg-gray-50 min-h-screen">
        <div className="bg-white border-b border-gray-200 px-4 py-3">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-xl font-semibold text-gray-900">Shipping Line</h1>
              <p className="text-xs text-gray-600 mt-0.5">
                Manage your supply chain shipping lines
              </p>
            </div>
          </div>
        </div>
  ...
  const handleSave = (data: { code: string; shippingLineCode: string; title: string }) => {
    if (editingShippingLine) {
      // Update existing shipping line
      setShippingLines((prev) =>
        prev.map((shippingLine) =>
          shippingLine.id === editingShippingLine.id
            ? { ...shippingLine, ...data }
            : shippingLine
        )
      );
    } else {
      // Create new shipping line with incremental ID
      const newShippingLine: ShippingLine = {
        id: idCounterRef.current++,
        code: data.code,
        shippingLineCode: data.shippingLineCode,
        title: data.title,
        createdAt: new Date(),
      };
  ...
  const handleDelete = (id: number) => {
    if (confirm("Are you sure you want to delete this shipping line?")) {
      setShippingLines((prev) => prev.filter((shippingLine) => shippingLine.id !== id));
    }
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Shipping Lines</h1>
            <p className="text-xs text-gray-600 mt-0.5">
              Manage your supply chain shipping lines
            </p>
          </div>
          ...
          {shippingLines.length === 0 ? (
            <div className="text-center py-8 px-4">
              ...
              <p className="text-xs text-gray-500 mb-4 max-w-sm mx-auto">
                Add shipping lines to organize your supply chain locations.
              </p>
              <ShippingLineDrawer
                isDrawerOpen={isDrawerOpen}
                setIsDrawerOpen={setIsDrawerOpen}
                  ...
                  <TableBody>
                    {shippingLines.map((shippingLine, index) => (
                      ...
                      >
                        ...
                        <TableCell className="px-4 py-2.5">
                          <div className="text-xs text-gray-500">
                            {shippingLine.createdAt.toLocaleDateString("en-US", {
                              month: "short",
                              day: "numeric",
                              year: "numeric",
                            })}
