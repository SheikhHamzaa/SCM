...
import ShippingStatusDrawer from "@/components/drawer/ShippingStatusDrawer";
...
export const ShippingStatus = () => {
  ...
  const [editingShippingStatus, setEditingShippingStatus] = useState<ShippingStatus | null>(null);
      ...
      <div className="bg-gray-50 min-h-screen">
        <div className="bg-white border-b border-gray-200 px-4 py-3">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-xl font-semibold text-gray-900">Shipping Status</h1>
              <p className="text-xs text-gray-600 mt-0.5">
                Manage your supply chain shipping statuses
              </p>
            </div>
          </div>
        </div>
  ...
  const handleSave = (data: { code: string; shippingStatusCode: string; title: string }) => {
    if (editingShippingStatus) {
      // Update existing shipping status
      setShippingStatuses((prev) =>
        prev.map((status) =>
          status.id === editingShippingStatus.id
            ? { ...status, ...data }
            : status
        )
      );
    } else {
      // Create new shipping status with incremental ID
      const newShippingStatus: ShippingStatus = {
        id: idCounterRef.current++,
        code: data.code,
        shippingStatusCode: data.shippingStatusCode,
        title: data.title,
        createdAt: new Date(),
      };
      setShippingStatuses((prev) => [...prev, newShippingStatus]);
    }

    // Reset and close drawer
    setEditingShippingStatus(null);
    setIsDrawerOpen(false);
  };

  const handleEdit = (shippingStatus: ShippingStatus) => {
    setEditingShippingStatus(shippingStatus);
    setIsDrawerOpen(true);
  };

  const handleDelete = (id: number) => {
    if (confirm("Are you sure you want to delete this shipping status?")) {
      setShippingStatuses((prev) => prev.filter((status) => status.id !== id));
    }
  };
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="flex justify-between items-center">
          ...
          {/* New Button */}
          <ShippingStatusDrawer
            isDrawerOpen={isDrawerOpen}
            setIsDrawerOpen={setIsDrawerOpen}
            onSave={handleSave}
            editingshippingStatus={editingShippingStatus}
          />
        </div>
      </div>
        ...
        <div className="bg-white rounded border border-gray-200 shadow-sm overflow-hidden">
          {shippingStatuses.length === 0 ? (
            <div className="text-center py-8 px-4">
              ...
              <h3 className="text-base font-medium text-gray-900 mb-1">
                No shipping statuses yet
              </h3>
              <p className="text-xs text-gray-500 mb-4 max-w-sm mx-auto">
                Add shipping statuses to organize your Purchases Shipping Status.
              </p>
              <ShippingStatusDrawer
                isDrawerOpen={isDrawerOpen}
                setIsDrawerOpen={setIsDrawerOpen}
              ...
              <div className="bg-[#F8F9FA] border-b border-gray-200 px-4 py-2">
                <div className="flex items-center justify-between">
                  <h2 className="text-xs font-semibold text-gray-900">
                    Shipping Statuses ({shippingStatuses.length})
                  </h2>
                  <div className="text-xs text-gray-500">
                    {shippingStatuses.length} items
                  </div>
                </div>
              </div>
